    ...
    def infer(self):
        ...
        for idx, image_file in enumerate(image_list):
            ...
            if len(batch_data) >= batch_size or idx == len(image_list) - 1:
                ...
                file_name = result[0]['file_name'].split('/')[2]
                class_ids = str(result[0]['class_ids'][0])
                batch_data.clear()
        ...
        #预测结果保存为CSV格式
        df.to_csv(self.config['Global']['save_inference_dir'], index=False, header=1)
        print("predict result save in './output/inference.csv'.")
